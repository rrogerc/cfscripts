...
function App() {
  ...
  return (
    <div className="min-h-screen flex flex-col bg-slate-900 text-slate-50 font-sans selection:bg-blue-500/30">
      {/* Header */}
      <header className="sticky top-0 z-10 bg-slate-900/80 backdrop-blur-md border-b border-slate-800 p-4 shadow-sm">
        <div className="max-w-2xl mx-auto flex items-center justify-between">
          <div className="flex items-center gap-2 text-blue-400">
            <BookOpen className="w-6 h-6" />
            <h1 className="text-xl font-bold tracking-tight text-white">CF Picker</h1>
          </div>
          
          <div className="flex items-center gap-3">
            <div className="flex items-center bg-slate-800 rounded-lg px-2 py-1 border border-slate-700 focus-within:border-blue-500 transition-colors">
              <span className="text-slate-400 text-sm font-medium mr-2">Lvl</span>
              <select 
                value={level} 
                onChange={(e) => setLevel(Number(e.target.value))}
                className="bg-transparent text-white font-semibold outline-none appearance-none cursor-pointer"
              >
      ...
      <main className="flex-1 w-full max-w-2xl mx-auto p-4 md:p-6 lg:py-8 flex flex-col">
        {loading ? (
          ...
          <div className="flex-1 flex flex-col items-center justify-center text-red-400 space-y-3 bg-red-950/20 p-6 rounded-2xl border border-red-900/50">
            <AlertCircle className="w-10 h-10" />
            <p className="text-center font-medium">{error}</p>
            <button onClick={fetchProblem} className="mt-4 px-4 py-2 bg-slate-800 hover:bg-slate-700 text-white rounded-lg transition-colors border border-slate-700">
              Try Again
            </button>
          </div>
        ) : html ? (
          <div className="animate-in fade-in slide-in-from-bottom-4 duration-500 pb-20">
            ...
            <div className="mb-6 pb-6 border-b border-slate-800">
              <div className="flex items-center gap-2 mb-2 text-sm font-medium text-slate-400">
                ...
                <span className="px-2 py-1 bg-blue-900/30 text-blue-400 rounded border border-blue-800/50">
            ...
            <div 
              ref={contentRef}
              className="problem-statement text-slate-200" 
          ...
          <div className="flex-1 flex flex-col items-center justify-center text-slate-500 space-y-4">
            ...
            <p className="text-lg font-medium text-center">Tap Pick to find a problem.</p>
      ...
      {html && !loading && (
        <div className="fixed bottom-6 left-0 right-0 flex justify-center pointer-events-none px-4 z-20">
          <button 
            onClick={fetchProblem}
            className="pointer-events-auto bg-slate-800 hover:bg-slate-700 text-white shadow-xl shadow-black/50 border border-slate-700 rounded-full px-6 py-3 flex items-center gap-2 font-medium tracking-wide transition-transform active:scale-95"
          >
